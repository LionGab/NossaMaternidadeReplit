...
// --- Load Auditors ---
const auditorNames = ["design-system"]; // Will add 'colors', 'a11y', 'tokens-reference', 'ux-review' later
const auditors = {};
...
auditorNames.forEach((name) => {
  try {
    ...
    // console.warn(`Auditor ${name} not yet implemented.`);
  }
});
      ...
      const color =
        v.severity === "critical"
          ...
          : v.severity === "high"
            ...
            : colors.blue;
      console.log(
        `  ${color}[${v.severity.toUpperCase()}]${colors.reset} ${v.message} ${colors.gray}(${v.ruleId}:${v.line})${colors.reset}`
      );
    });
  });
